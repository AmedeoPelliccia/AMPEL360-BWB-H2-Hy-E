# ATA 02 - Operations Information  
## Regulatory Framework

**Document ID:** AMPEL360-02-00-00-OVR-RF  
**Version:** 1.0.0  
**Date:** 2025-11-04  
**Classification:** Operations Critical

---

## 1. PURPOSE

This document outlines the regulatory framework governing operations of the AMPEL360 BWB H2 Hy-E Q100 INTEGRA aircraft, ensuring compliance with all applicable aviation regulations.

---

## 2. PRIMARY REGULATORY AUTHORITIES

### 2.1 European Union Aviation Safety Agency (EASA)

**Authority:** Primary certification authority  
**Jurisdiction:** European Union member states  
**Website:** [www.easa.europa.eu](https://www.easa.europa.eu)

**Key Regulations:**
- **[CS-25](https://www.easa.europa.eu/document-library/certification-specifications/cs-25-large-aeroplanes)** (Certification Specifications for Large Aeroplanes)
- **[EU Regulation 965/2012](https://www.easa.europa.eu/document-library/regulations/regulation-eu-no-9652012)** (Air Operations)
- **Part-ORO** (Organisation Requirements for Air Operations)
- **Part-CAT** (Commercial Air Transport)

### 2.2 Federal Aviation Administration (FAA)

**Authority:** Validation authority (US operations)  
**Jurisdiction:** United States  
**Website:** [www.faa.gov](https://www.faa.gov)

**Key Regulations:**
- **[14 CFR Part 25](https://www.ecfr.gov/current/title-14/chapter-I/subchapter-C/part-25)** (Airworthiness Standards: Transport Category Airplanes)
- **[14 CFR Part 121](https://www.ecfr.gov/current/title-14/chapter-I/subchapter-G/part-121)** (Operating Requirements: Domestic, Flag, and Supplemental Operations)
- **[14 CFR Part 91](https://www.ecfr.gov/current/title-14/chapter-I/subchapter-F/part-91)** (General Operating and Flight Rules)

### 2.3 International Civil Aviation Organization (ICAO)

**Authority:** International standards setting  
**Jurisdiction:** Global (193 member states)  
**Website:** [www.icao.int](https://www.icao.int)

**Key Standards:**
- **[Annex 6](https://www.icao.int/safety/airnavigation/Operations/Pages/AnnexCertificationSpecifications.aspx)** (Operation of Aircraft)
- **[Annex 8](https://www.icao.int/safety/airnavigation/NationalityMarks/Annexes_Booklet_en.pdf)** (Airworthiness of Aircraft)
- **Doc 8168** (PANS-OPS)
- **Doc 9859** (Safety Management Manual)

---

## 3. CERTIFICATION BASIS

### 3.1 Type Certificate

**Certification Specification:** CS-25 Amendment 27  
**FAA Equivalent:** 14 CFR Part 25 Amendment 25-145

**Certification Date:** [Pending]  
**Certificate Holder:** AMPEL360 Aerospace

### 3.2 Special Conditions

**Hydrogen Fuel System:**
- EASA Special Condition SC-H2-001 (pending)
- Cryogenic fuel storage
- Leak detection requirements
- Fire protection systems

**Fuel Cell Propulsion:**
- EASA Special Condition SC-FC-001 (pending)
- Electric propulsion systems
- Thermal management requirements
- Emergency power provisions

**AI/ML Systems (CAOS):**
- EASA AI Roadmap 2.0 compliance
- EU AI Act (High-Risk Systems)
- Transparency and explainability requirements
- Human oversight mandates

**BWB Configuration:**
- Non-conventional aerodynamic design
- Unique weight and balance characteristics
- Special handling qualities
- Non-standard flight deck visibility

### 3.3 Operational Approvals

**Required Operational Approvals:**
- ETOPS 180-minute approval
- Cat II/III approach operations
- RNP AR (Required Navigation Performance - Authorization Required)
- PBN (Performance-Based Navigation)
- RVSM (Reduced Vertical Separation Minima)

---

## 4. AIRWORTHINESS REGULATIONS

### 4.1 CS-25 / FAR 25 Requirements

**Subpart G - Operating Limitations and Information**

| Regulation | Requirement | ATA 02 Compliance |
|------------|-------------|-------------------|
| CS 25.1581 | General | 02-00-00 General documentation |
| CS 25.1583 | Operating limitations | 02-50-00 Operating limitations |
| CS 25.1585 | Operating procedures | 02-80-00 Operational procedures |
| CS 25.1587 | Performance information | 02-40-00 Performance data |
| CS 25.1589 | Loading information | 02-20-00 Weight and balance |

**Aircraft Flight Manual (AFM) Requirements:**
- Section 1: General
- Section 2: Limitations
- Section 3: Emergency Procedures
- Section 4: Normal Procedures
- Section 5: Performance
- Section 6: Weight and Balance
- Section 7: Systems Description

### 4.2 Continuing Airworthiness

**Part-26 / Part 26 Requirements:**
- Widespread Fatigue Damage (WFD)
- Damage Tolerance Assessment
- Fuel Tank Safety
- Systems Safety Assessment

**Maintenance Program Approval:**
- Maintenance Review Board (MRB) report
- Maintenance Planning Document (MPD)
- Minimum Equipment List (MEL/MMEL)
- Configuration Deviation List (CDL)

---

## 5. OPERATIONS REGULATIONS

### 5.1 EU Regulation 965/2012 (Air Operations)

**Part-ORO (Organisation Requirements):**
- ORO.GEN: General requirements
- ORO.AOC: Air operator certificate
- ORO.FC: Flight crew
- ORO.CC: Cabin crew
- ORO.DEC: Declared training organisations

**Part-CAT (Commercial Air Transport):**
- CAT.GEN: General requirements
- CAT.OP: Operating procedures
- CAT.POL: Performance and operating limitations
- CAT.IDE: Instruments, data and equipment

**Part-SPA (Special Operations):**
- SPA.ETOPS: Extended range operations
- SPA.LVO: Low visibility operations
- SPA.PBN: Performance-based navigation
- SPA.MNPS: Minimum navigation performance specifications

### 5.2 FAA Part 121 (Air Carriers)

**Subpart K - Instrument and Equipment Requirements:**
- §121.305: Flight and navigational equipment
- §121.306: Portable electronic devices
- §121.333: Flight recorders
- §121.359: Cockpit voice recorders

**Subpart V - Records and Reports:**
- §121.683: Crewmember and dispatcher records
- §121.685: Aircraft record: Commercial operators
- §121.687: Dispatch records

**Subpart W - Maintenance:**
- §121.363: General maintenance requirements
- §121.365: Maintenance programs
- §121.369: Manual requirements

---

## 6. HYDROGEN-SPECIFIC REGULATIONS

### 6.1 Hydrogen Fuel Standards

**ISO Standards:**
- **[ISO 19881](https://www.iso.org/standard/66845.html)**: Gaseous hydrogen - Land vehicle fuel containers
- **[ISO 19880-1](https://www.iso.org/standard/71940.html)**: Gaseous hydrogen - Fueling stations - Part 1: General requirements
- **[ISO 14687](https://www.iso.org/standard/69539.html)**: Hydrogen fuel quality - Product specification

**SAE Standards:**
- **[SAE J2719](https://www.sae.org/standards/content/j2719_201109/)**: Hydrogen Fuel Quality for Fuel Cell Vehicles
- **[SAE J2601](https://www.sae.org/standards/content/j2601_201612/)**: Fueling Protocols for Light Duty Gaseous Hydrogen Surface Vehicles
- **[SAE J2578](https://www.sae.org/standards/content/j2578_201408/)**: Recommended Practice for General Fuel Cell Vehicle Safety
- **SAE J2601**: Fueling Protocols for Light Duty Gaseous Hydrogen Surface Vehicles
- **SAE J2578**: Recommended Practice for General Fuel Cell Vehicle Safety

### 6.2 Cryogenic Standards

**EN Standards:**
- **EN 13458**: Cryogenic vessels - Static vacuum insulated vessels
- **EN 14025**: Cryogenic vessels - Metallic inner parts
- **EN 12493**: LH2 land vehicle fuel system

### 6.3 EASA Hydrogen Guidance

**EASA Type Certification:**
- Special Condition for Hydrogen Fuel Systems (pending)
- Cryogenic Storage Requirements
- Leak Detection and Fire Protection
- Ground Handling Safety

**Compliance Demonstration:**
- Safety assessment (ARP4761)
- Failure modes and effects analysis (FMEA)
- Fault tree analysis (FTA)
- Common cause analysis

---

## 7. AI/ML REGULATIONS

### 7.1 EU AI Act

**Classification:** High-Risk AI System (Aviation Safety)

**Requirements:**
- Risk management system
- Data governance
- Technical documentation
- Transparency and provision of information to users
- Human oversight
- Accuracy, robustness, and cybersecurity

**Reference:** [EU AI Act (Regulation 2024/1689)](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32024R1689)

**Compliance for CAOS:**
- Documented risk assessment
- Training data quality requirements
- Model validation and verification
- Explainability of recommendations
- Human-in-the-loop authority
- Continuous monitoring

### 7.2 EASA AI Roadmap

**[EASA AI Roadmap 2.0 (2023)](https://www.easa.europa.eu/en/domains/innovation-digitalization/artificial-intelligence/ai-roadmap):**
- Level 1: AI Assistance to Humans (CAOS is here)
- Level 2: AI Human Assistance
- Level 3: AI Automation
- Level 4: Full AI Automation

**Certification Approach for Level 1:**
- Functional hazard assessment (FHA)
- System safety assessment (SSA)
- Software level determination
- Verification and validation
- Human factors assessment

### 7.3 FAA AI Assurance Framework

**FAA Trust AI Framework:**
- AI system characterization
- Risk-based approach
- Verification and validation methods
- Ongoing monitoring and learning
- Human-machine interface design

---

## 8. ENVIRONMENTAL REGULATIONS

### 8.1 Emissions

**ICAO Annex 16, Volume II:**
- CO₂ emissions standards
- Certification of emissions
- Operational measures

**CORSIA (Carbon Offsetting and Reduction Scheme):**
- Baseline emissions
- Offsetting requirements
- Monitoring and reporting

**AMPEL360 Compliance:**
- Zero direct CO₂ emissions (H2 combustion → H2O)
- Net negative with CO₂ capture system
- Exceeds all current and proposed emission standards

### 8.2 Noise

**ICAO Annex 16, Volume I:**
- Noise certification standards
- Chapter 4 (current) / Chapter 14 (proposed)

**AMPEL360 Compliance:**
- 50% noise reduction vs conventional
- Exceeds Chapter 14 proposed standards
- Distributed propulsion benefits

---

## 9. OPERATIONAL APPROVALS

### 9.1 ETOPS (Extended Range Operations)

**Requirement:** ETOPS 180 approval for trans-oceanic routes

**Compliance Requirements:**
- Proven reliability (fuel cells, H2 system)
- Two-communication systems
- Crew training (ETOPS-specific)
- Maintenance program enhancements
- MEL considerations

**AMPEL360 ETOPS:**
- Target: ETOPS 180 at entry into service
- Fuel cell redundancy (4 stacks, 3 required)
- H2 system redundancy
- CAOS predictive maintenance

### 9.2 Low Visibility Operations (LVO)

**Approvals:** CAT II and CAT III

**Requirements:**
- Aircraft equipment (autoland, HUD)
- Ground equipment (ILS Cat II/III)
- Crew training and qualification
- Operations manual procedures
- Maintenance program

**AMPEL360 LVO:**
- CAT IIIb capable (RVR 75m)
- Dual HUD (Head-Up Display)
- CAOS-assisted approach monitoring
- Auto-land capable

### 9.3 Performance-Based Navigation (PBN)

**Capabilities:**
- RNP 0.3 (Terminal)
- RNP 1 (En-route)
- RNP AR (Authorization Required)
- RNP APCH (Approach)

**Benefits:**
- Optimized routing
- CAOS route optimization
- Reduced ATC workload
- Fuel efficiency

---

## 10. SAFETY MANAGEMENT

### 10.1 Safety Management System (SMS)

**ICAO Annex 19 Requirements:**
- Safety policy and objectives
- Safety risk management
- Safety assurance
- Safety promotion

**SMS Implementation:**
- Operator responsibility
- Integration with CAOS
- Continuous monitoring
- Safety performance indicators

### 10.2 Safety Reporting

**Mandatory Reporting:**
- Accidents
- Serious incidents
- TCAS RA (Resolution Advisory)
- H2 leaks or fires
- Fuel cell failures

**Voluntary Reporting:**
- ASAP (Aviation Safety Action Program)
- FOQA (Flight Operations Quality Assurance)
- CAOS anomaly detection

---

## 11. TRAINING AND COMPETENCY

### 11.1 Crew Training Requirements

**Type Rating:**
- EASA Part-FCL.720.A
- FAA 14 CFR 61.31(a)

**Training Content:**
- BWB handling (5 hours)
- H2 systems (3 hours)
- Fuel cell operations (2 hours)
- CAOS operations (3 hours)
- Emergency procedures (4 hours)

**Simulator Requirements:**
- Full flight simulator (Level D)
- BWB-specific handling
- H2 emergency scenarios
- Fuel cell failure scenarios
- CAOS failure and degraded modes

### 11.2 Ground Personnel Training

**H2 Refueling Personnel:**
- Certification required (8 hours initial)
- Cryogenic handling
- Safety procedures
- Emergency response
- Annual recurrent (4 hours)

**Maintenance Personnel:**
- Type training for H2 system
- Type training for fuel cells
- CAOS system training
- Special tools and equipment

---

## 12. COMPLIANCE MONITORING

### 12.1 Continuing Surveillance

**EASA/FAA Oversight:**
- Annual safety audits
- Ramp inspections
- Records reviews
- Performance monitoring

**Operator Responsibilities:**
- Internal audits
- Safety performance monitoring
- Compliance tracking
- Corrective action programs

### 12.2 Regulatory Changes

**Monitoring:**
- Subscribe to regulatory updates
- Industry working groups
- Manufacturer support
- Compliance impact assessment

**Implementation:**
- Gap analysis
- Action plan development
- Training updates
- Documentation revisions

---

## 13. INTERNATIONAL OPERATIONS

### 13.1 Foreign Operations

**Requirements:**
- Validation of type certificate
- Operations specifications
- Crew licensing validation
- H2 infrastructure availability

**Key Markets:**
- European Union (EASA member states)
- United States (FAA)
- United Kingdom (CAA)
- Canada (Transport Canada)
- Australia (CASA)

### 13.2 Bilateral Agreements

**EASA-FAA Agreement:**
- Type certificate validation
- Continued airworthiness
- Operations oversight

**Other Agreements:**
- EASA-Transport Canada
- EASA-CAA UK
- FAA-CASA Australia

---

## 14. COMPLIANCE DOCUMENTATION

### 14.1 Required Documents

**Type Certificate Data Sheet (TCDS):**
- Airworthiness limitations
- Certification basis
- Equipment list
- Model variations

**Aircraft Flight Manual (AFM):**
- EASA/FAA approved
- Limitations section
- Emergency procedures
- Performance data

**Minimum Equipment List (MMEL/MEL):**
- Master MEL (manufacturer)
- Operator MEL (specific)
- Dispatch deviations
- Operational procedures

### 14.2 Maintenance Documentation

**Maintenance Planning Document (MPD):**
- Scheduled maintenance tasks
- Inspection intervals
- H2 system maintenance
- Fuel cell maintenance

**Maintenance Review Board Report (MRBR):**
- MSG-3 analysis
- Task development
- Intervals and thresholds
- Airworthiness limitations

---

## 15. CONTACT INFORMATION

**Regulatory Affairs:**
- Email: regulatory@ampel360.aero
- Phone: +34 91 XXX XXXX (office hours)

**Certification:**
- Email: certification@ampel360.aero
- Phone: +34 91 XXX XXXX (office hours)

**Compliance:**
- Email: compliance@ampel360.aero
- Phone: +34 91 XXX XXXX (office hours)

**EASA Liaison:**
- Email: easa-liaison@ampel360.aero

**FAA Liaison:**
- Email: faa-liaison@ampel360.aero

---

**Document Status:** ✅ RELEASED  
**Effective Date:** 2029-01-01 (Entry Into Service)  
**Next Review:** 2026-11-04 (Annual)  
**Configuration Control:** Git SHA-256: [hash]
