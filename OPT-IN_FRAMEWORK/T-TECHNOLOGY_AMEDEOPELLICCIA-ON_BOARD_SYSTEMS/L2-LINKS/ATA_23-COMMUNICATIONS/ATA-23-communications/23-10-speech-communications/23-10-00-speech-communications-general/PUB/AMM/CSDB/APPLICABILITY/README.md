...
This directory contains **Applicability** definitions and cross-reference files for the Aircraft Maintenance Manual (AMM). Applicability defines which technical content applies to which aircraft variants, configurations, modifications, and serial numbers.
...
<applic>
  <evaluate andOr="and">
    <assert applicPropertyIdent="MODEL" applicPropertyValues="Q100"/>
    <assert applicPropertyIdent="MOD" applicPropertyValues="MOD-23-001-NOT-INSTALLED"/>
  </evaluate>
</applic>
```
...
<para>
  <applic>
    <assert applicPropertyIdent="OPTION" applicPropertyValues="HF_RADIO"/>
  </applic>
  This step applies only to aircraft with HF Radio option.
</para>
```
...
**Standard**:
- Baseline communications systems
- Standard VHF radios
- Basic audio panel
...
**Cargo**:
- Reduced communications options
- Crew communications only
- Modified intercom
...
<applic>
  <evaluate andOr="or">
    <assert applicPropertyIdent="MOD" applicPropertyValues="BEFORE_MOD_23_100"/>
    <assert applicPropertyIdent="MOD" applicPropertyValues="AFTER_MOD_23_100"/>
  </evaluate>
</applic>
```
