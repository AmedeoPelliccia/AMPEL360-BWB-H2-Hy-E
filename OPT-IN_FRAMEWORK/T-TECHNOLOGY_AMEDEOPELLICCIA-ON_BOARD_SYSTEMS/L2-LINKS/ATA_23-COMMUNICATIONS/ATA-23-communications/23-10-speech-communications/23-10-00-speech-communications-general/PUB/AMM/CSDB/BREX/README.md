...
This directory contains **Business Rules Exchange (BREX)** files for the Aircraft Maintenance Manual (AMM). BREX files define project-specific validation rules that extend and constrain the base S1000D specification.
...
<notationRule>
  ...
  <pattern>AMPEL360-23-\d{2}-\d{2}-\d{3}[A-Z]-[A-Z]-[A-Z]{1,3}</pattern>
</notationRule>
```
