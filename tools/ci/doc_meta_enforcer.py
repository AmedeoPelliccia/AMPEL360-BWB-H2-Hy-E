...
def find_component_root(path: pathlib.Path) -> pathlib.Path:
    """
    Given a file like
    ...
    p = path
    while p != REPO_ROOT:
        if re.match(r"\d{2}-\d{2}-\d{2}_.+", p.name):
            return p
        p = p.parent
    return DOC_ROOT
...
def process_file(path: pathlib.Path, fix: bool) -> List[str]:
    """
    Process a single markdown file.
    Returns a list of issues (strings) if in check mode or if something was fixed.
    """
    text = path.read_text(encoding="utf-8")
    component_root = find_component_root(path)
...
def main() -> int:
    ...
    for md in DOC_ROOT.rglob("*.md"):
        issues = process_file(md, fix=fix)
